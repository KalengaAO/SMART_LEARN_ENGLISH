...
struct curl_slist *build_header( void ){
...
std::string	build_json(std::string prompt){
	json	body;
	string	res;
...
std::string	janela::requisition(std::string const &prompt){
	...
	std::string	response;
	std::string	body_json = build_json(prompt);

	struct	curl_slist *header = build_header();
	if (!header)
